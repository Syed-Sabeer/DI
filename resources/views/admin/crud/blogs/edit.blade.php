...
<form class="row g-3 common-form" method="POST" action="{{ $isEdit ? route('admin.blog.update', $blog->id) : route('admin.blog.store') }}" enctype="multipart/form-data">
  @csrf
  @if ($isEdit)
    @method('PUT')
  @endif
  ...
  <div class="col-md-12">
    ...
    <input class="form-control" name="slug" type="text" placeholder="blog-slug" value="{{ old('slug', $blog->slug ?? '') }}">
  </div>
  ...
  <div class="col-md-6">
    ...
    <div class="input-group"><select class="form-select blog-category-select" name="category" id="blog-category-select"><option value=""></option>@foreach(($categories ?? []) as $category)<option value="{{ $category }}" @selected(old('category', $blog->category ?? '') === $category)>{{ $category }}</option>@endforeach</select><button class="btn btn-outline-primary" type="button" data-bs-toggle="modal" data-bs-target="#categoryModal"><i class="fa fa-plus"></i></button></div>
    <datalist id="blog-categories">@foreach(($categories ?? []) as $category)<option value="{{ $category }}">@endforeach</datalist>
  </div>

  <div class="col-md-6">
    <label class="form-label">Tags</label>
    <input type="hidden" name="tags" id="blog-tags-value" value="{{ old('tags', $blog->tags ?? '') }}">
  ...
  <div class="col-md-12">
    ...
    <input class="form-control" name="meta_title" type="text" maxlength="255" placeholder="Meta title for search engines" value="{{ old('meta_title', $blog->meta_title ?? '') }}">
  </div>
  ...
  <div class="col-md-12">
    ...
    <textarea class="form-control" name="meta_description" maxlength="320" rows="2" placeholder="Short summary shown in search results">{{ old('meta_description', $blog->meta_description ?? '') }}</textarea>
  </div>

  <div class="col-md-12">
    <label class="form-label">Meta Keywords <small class="text-muted">(optional, comma separated)</small></label>
    <input class="form-control" name="meta_keywords" type="text" maxlength="255" placeholder="e.g. software development, mobile apps" value="{{ old('meta_keywords', $blog->meta_keywords ?? '') }}">
  </div>
      ...
      <div class="d-flex align-items-start gap-3 position-relative" style="z-index: 1;">
        ...
        <div class="flex-grow-1">
          <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
            <div>
              <label class="form-label text-white fw-bold fs-6 mb-1" for="send-newsletter">Newsletter subscribers</label>
            ...
            <div class="form-check form-switch m-0">
              <input type="hidden" name="newsletter_recipients_enabled" value="0">
              <input class="form-check-input" id="send-newsletter" name="newsletter_recipients_enabled" type="checkbox" value="1" role="switch" @checked(old('newsletter_recipients_enabled', $newsletterRecipientsEnabled)) @disabled($subscriberCount === 0)>
            </div>
          </div>
          <div class="mt-3" id="newsletter-options">
            ...
            <select name="newsletter_audience" id="newsletter-audience" class="form-select">
              <option value="all" @selected(old('newsletter_audience', $newsletterAudience) === 'all')>All subscribers</option>
              <option value="selected" @selected(old('newsletter_audience', $newsletterAudience) === 'selected')>Specific subscribers</option>
            </select>
            <div id="newsletter-recipients" class="mt-3" style="color:#212529">
              ...
              <select name="newsletter_subscriber_ids[]" id="newsletter-subscriber-ids" class="form-select" multiple>
                @foreach($subscribers as $subscriber)
                  <option value="{{ $subscriber->id }}" @selected(in_array($subscriber->id, array_map('intval', (array) old('newsletter_subscriber_ids', $selectedSubscriberIds))))>{{ $subscriber->email }}</option>
                @endforeach
              </select>
            </div>
          </div>
          <div class="newsletter-broadcast-meta mt-2">
            ...
            Saving only updates the subscriber selection and does not send any email. Use the Resend button on the blog list whenever you want to send it again.
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      document.querySelectorAll('[data-tags-input]').forEach(function (wrapper) {
        ...
        function render() { list.innerHTML = tags.map((tag, i) => `<span class="badge bg-primary">${tag}<button type="button" class="btn-close btn-close-white ms-1" data-remove="${i}" aria-label="Remove"></button></span>`).join(''); hidden.value = tags.join(', '); }
        entry.addEventListener('keydown', function (event) { if (event.key === 'Enter' || event.key === ',') { event.preventDefault(); const tag = entry.value.trim().replace(/,$/, ''); if (tag && !tags.includes(tag)) tags.push(tag); entry.value = ''; render(); } });

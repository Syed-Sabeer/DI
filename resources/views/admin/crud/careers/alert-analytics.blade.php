...
@section('title', 'Job Alert Analytics')
  ...
  <div class="container-fluid">
    <div class="page-title">
      <div class="row align-items-center">
        <div class="col-sm-8">
          <h3>Job Alert Analytics</h3>
          <p class="text-muted mb-0">{{ $career->job_title }}@if($career->location) &middot; {{ $career->location }}@endif</p>
        </div>
        <div class="col-sm-4 text-sm-end mt-3 mt-sm-0">
          <a class="btn btn-light" href="{{ route('admin.careers.index') }}"><i class="fa fa-arrow-left me-1"></i> Careers</a>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="container-fluid">
    <div class="row g-3 mb-4">
      @foreach([
        ...
        ['Unique Job Viewers', (int) $summary->viewed_count, 'primary'],
        ['Total Open Events', (int) $summary->total_open_events, 'info'],
        ['Total Job View Events', (int) $summary->total_view_events, 'warning'],
        ['Applications Received', (int) $applicationCount, 'success'],
      ...
      <div class="col-xl-3 col-md-4 col-sm-6"><div class="card h-100"><div class="card-body"><p class="text-muted mb-1">Click-through Rate</p><h3 class="mb-0">{{ number_format($viewRate, 1) }}%</h3></div></div></div>
    </div>
    ...
    <div class="card" data-live-submissions>
      <div class="card-header card-no-border pb-3">
        <div class="row g-3 align-items-end">
          <div class="col-12">
            <h5 class="mb-1">Alert Deliveries</h5>
          ...
          <div class="col-12">
            <form method="GET" action="{{ route('admin.careers.alert-analytics', $career) }}" class="row g-2 align-items-end" data-live-filter-form>
              ...
              <div class="col-xl-2 col-lg-3 col-md-4">
                ...
                <select class="form-select" id="engagement-filter" name="engagement">
                  ...
                  <option value="viewed" @selected(request('engagement') === 'viewed')>Job viewed</option>
                  <option value="not_viewed" @selected(request('engagement') === 'not_viewed')>Job not viewed</option>
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 d-flex gap-2">
                ...
                @if(request()->filled('search') || request('engagement', 'all') !== 'all' || request('status', 'all') !== 'all' || request('sort', 'latest') !== 'latest')
                  <a class="btn btn-light" href="{{ route('admin.careers.alert-analytics', $career) }}" title="Clear filters"><i class="fa fa-times"></i></a>
                @endif
              </div>
            </form>
          </div>
      ...
      <div class="card-body px-0 pt-0">
        <div class="table-responsive">
          <table class="table align-middle mb-0">
            <thead><tr><th>Subscriber Email</th><th>Delivery Status</th><th>Sent At</th><th>Open Detected</th><th>Open Count</th><th>Job Viewed</th><th>View Count</th><th>Last Activity</th></tr></thead>
            <tbody>
              ...
              @empty
                <tr><td colspan="8" class="text-center py-5 text-muted">No job alerts have been sent for this opening yet.</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
    ...
    <p class="text-muted small mt-3">Open rate and click-through rate use successfully sent deliveries as the denominator. Email open detection may be affected by image blocking or proxying. "Applications Received" counts every application for this role, including candidates who did not arrive from the email.</p>
  </div>
</div>
@endsection

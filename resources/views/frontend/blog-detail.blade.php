...
@section('meta_image', ($blog->image && file_exists(public_path('storage/'.$blog->image))) ?
      ...
      <div class="col-xl-8">
        <article class="article-shell article-details">
          <header class="article-head">
            <h2 class="article-title split-title">{{ $blog->title }}</h2>
          </header>
          <div class="post-facts wow fadeInUp" data-wow-delay=".3s">
            ...
            <div class="post-facts__cell">
              ...
              <div class="post-facts__text"><span class="post-facts__label">Category</span>
                <div class="post-facts__value"><a href="{{ route('blog', ['category' => $blog->category]) }}">{{
                    $blog->category }}</a></div>
              </div>
            </div>
            @endif
            <div class="post-facts__cell">
              ...
              <div class="post-facts__text"><span class="post-facts__label">Date Released</span>
                <div class="post-facts__value"><time datetime="{{ optional($blog->created_at)->toDateString() }}">{{
                    optional($blog->created_at)->format('d F, Y') }}</time></div>
              </div>
            </div>
            @if($blog->min_read)
            <div class="post-facts__cell">
              <div class="post-facts__icon"><i class="ri-time-line"></i></div>
              <div class="post-facts__text"><span class="post-facts__label">Reading Time</span>
                <div class="post-facts__value">{{ $blog->min_read }}</div>
              </div>
            </div>
            @endif
          </div>
      ...
      <div class="col-xl-4">
              ...
              <article class="mini-post">
                ...
                <div class="mini-post__body">
                  <h3 class="mini-post__heading"><a class="mini-post__link"
                      ...
                      48) }}</a></h3><time class="mini-post__date"
                    datetime="{{ optional($latest->created_at)->toDateString() }}">{{
        ...
        <aside class="aside-panel mb-4">
          <div class="side-card side-nav wow fadeInUp" data-wow-delay=".3s">
            ...
            <ul class="blog-category-list">
              ...
              <li><a href="{{ route('blog', ['category' => $category->category]) }}">{{ $category->category }} <span
                    class="blog-count">({{ str_pad($category->total, 2, '0', STR_PAD_LEFT) }})</span></a></li>
              @endforeach
            </ul>
          </div>
        </aside>
        ...
        <aside class="aside-panel mb-4">
          <div class="side-card side-nav wow fadeInUp" data-wow-delay=".4s">
            ...
            <div class="tags-container">@foreach($tags as $tag)<a href="{{ route('blog', ['search' => $tag]) }}">{{ $tag
                }}</a>@endforeach</div>
          </div>
        </aside>
        @endif

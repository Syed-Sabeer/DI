...
<style>
    ...
    /* ---------- Cover image ----------
       Framed as an app window (echoing the hero breadcrumb chrome) and lifted
       up so it straddles the hero wave, the way a case-study cover should. */
    .portfolio-cover {
        position: relative;
        z-index: 2;
        max-width: 880px;
    ...
    .portfolio-cover__dots {
        display: inline-flex;
        gap: 6px;
        flex: 0 0 auto;
    }

    .portfolio-cover__dots span {
        width: 9px;
        height: 9px;
        border-radius: 50%;
    ...
    .portfolio-cover__dots span:first-child {
        background: var(--accent);
    }

    .portfolio-cover__path {
        flex: 1 1 auto;
        min-width: 0;
        overflow: hidden;
        white-space: nowrap;
    ...
    .portfolio-cover__path b {
        font-weight: 600;
        color: rgba(255, 255, 255, 0.78);
    }

    .portfolio-cover__year {
        flex: 0 0 auto;
        font-family: ui-monospace, SFMono-Regular, "SF Mono", Menlo, Consolas, monospace;
        font-size: 0.7rem;
...
<section class="section service-article section-gap">
    <div class="container">
        <figure class="portfolio-cover wow fadeInUp" data-wow-delay=".1s" style="--accent:{{ $portfolio['accent'] }};">
            <div class="portfolio-cover__frame">
                <div class="portfolio-cover__bar">
                    <span class="portfolio-cover__dots"><span></span><span></span><span></span></span>
                    <span class="portfolio-cover__path">~/portfolio/<b>{{ $portfolio['slug'] }}</b></span>
                    @if(!empty($portfolio['year']))
                    <span class="portfolio-cover__year">{{ $portfolio['year'] }}</span>
                    @endif
                </div>
